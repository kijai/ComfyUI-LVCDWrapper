...
class CrossAttention_Masked(nn.Module):
    def __init__(
        self,
        query_dim,
        context_dim=None,
    ...
    def _forward(
        ...
        h = self.heads
        if mask is None:
            mask = torch.ones(x.shape[-2], x.shape[-2]).to(x)
...
class CrossAttention_Cond(nn.Module):
    def __init__(
        self,
        query_dim,
        context_dim=None,
    ...
    def _forward(
        ...
        _, _, R, C = context.shape
        context = rearrange(context, 'b c h w -> b (h w) c')

        # Positional embedding
        row_emb = repeat(self.row_emb[:, :R], 'n r d -> n (r c) d', c=C)
        col_emb = repeat(self.col_emb[:, :C], 'n c d -> n (r c) d', r=R)
...
class SelfAttentionRefconcatFirst(nn.Module):
    def __init__(
        self,
        query_dim,
        context_dim=None,
...
class SelfAttentionRefonlyFirst(nn.Module):
    def __init__(
        self,
        query_dim,
        context_dim=None,
    ...
    def _forward(
        ...
        context = rearrange(context, '(b t) ... -> b t ...', t=T)[:, [0]]

        k = self.to_k(context)
        v = self.to_v(context)

        k, v = map(lambda t: repeat(t, 'b t0 ... -> b (t t0) ...', t=T), (k, v))
        k, v = map(lambda t: rearrange(t, 'b t ... -> (b t) ...'), (k, v))
...
class BasicTransformerBlock(nn.Module):
    ATTENTION_MODES = {
        ...
        "softmax-refconcat-first": SelfAttentionRefconcatFirst,
        "softmax-refonly-first": SelfAttentionRefonlyFirst,
    }
    ...
    def __init__(
        ...
        if spatial_self_attn_type is None:
            selfattn_cls = attn_cls
        elif isinstance(spatial_self_attn_type, str): 
            self.ATTENTION_MODES[spatial_self_attn_type]
        else:
            selfattn_cls = spatial_self_attn_type
        ...
        self.addtional_attn_cond = additional_attn_cond
        if additional_attn_cond:
            self.attn3 = CrossAttention_Cond(
                query_dim=dim,
                context_dim=dim,
                heads=n_heads,
                dim_head=d_head,
    ...
    def forward(
        self, x, context=None, attn_cond=None, additional_tokens=None, n_times_crossframe_attn_in_self=0
    ):
        kwargs = {"x": x}
        ...
        if attn_cond is not None:
            kwargs.update({"attn_cond": attn_cond})
        ...
        if self.checkpoint:
            ...
            if attn_cond is None:
                ...
                return checkpoint(self._forward, x, context, attn_cond)
        else:
    ...
    def _forward(
        ...
        out2 = self.attn2( self.norm2(x), context=context, additional_tokens=additional_tokens )

        x = out2 + x

        if self.addtional_attn_cond and attn_cond is not None:
            a = self.attn3(x, context=attn_cond)
            x = a + x
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                BasicTransformerBlock(
                    ...
                    additional_attn_cond=additional_attn_cond,
                    spatial_self_attn_type=spatial_self_attn_type,
                )
